...
* Data Generation will be used for advancements, loot-tables, recipes, tags, models, translations & some other stuff (where possible)
* A lot of Models will be done with Data Generation, but complex Models will remain as JSON Files
* Don't forget to add EMI compatibility once they port to whatever version we are at
...
* The Mod Logo is basically just a copy of the [NTM-Wiki](https://nucleartech.wiki) Logo
...
When a Node is placed it checks it neighbor blocks for Nodes, if it finds one then this Node will use its Network & if it finds multiple, it will merge all the Networks Together.
...
OBJ Files are loaded from the Resource Manager.
