    ...
    public @NonNull ItemStack finishUsingItem(@NonNull ItemStack stack, @NonNull Level world, @NonNull LivingEntity user) {
        if (world instanceof ServerLevel serverWorld) {
            if (!user.hasInfiniteMaterials()) {
                NtmEntityUtil.applyDamage(user, serverWorld, NtmDamageTypes.EUTHANIZED, Float.MAX_VALUE);
            } else {
                NtmEntityUtil.applyDamage(user, serverWorld, NtmDamageTypes.EUTHANIZED, 10);
            }
        }
